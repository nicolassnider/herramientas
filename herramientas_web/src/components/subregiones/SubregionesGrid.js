...
class SubRegionesGrid extends Component {
	constructor(props) {
		super(props);
        this.dataService = '/subregiones/grid';
				...
				columns: [
					...
					drawCallback: function () {
						$(this).find('.action').on('click', function () {
							...
							.then((isConfirm) => {
								if (isConfirm) {
									fetch(Config.get('apiUrlBase') + '/subregiones/' + data.id, {
										method: 'DELETE',
										headers: {
			    					...
			    					<table id="dataTable" className="table nowrap server-side table-hover" ref="grid" width="100%">
											<tfoot>
												<tr style={{backgroundColor: '#fff'}}>
													...
													<th className=""></th>
													<th className="p-0"></th>
													<th className="dt-search-header">Región</th>
													<th className="dt-search-header">País</th>
...
export default SubRegionesGrid;
