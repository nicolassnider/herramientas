				...
				columns: [
					...
					drawCallback: function () {
						$(this).find('.action').on('click', function () {
							...
							.then((isConfirm) => {
								if (isConfirm) {
									fetch(Config.get('apiUrlBase') + '/marcas/' + data.id, {
										method: 'DELETE',
										headers: {
					...
					rowCallback: function(row, data) {
						if (data.foto) {
							let photoUrl = Config.get('apiUrlBase') + '/marcas/foto/' + data.foto;
							fetch(photoUrl, {
								method: 'GET',
								headers: {
									'Authorization-Token': localStorage.getItem("token")
								...
								if (response.status === 200) {
									...
									$('#avatar', row).html(data.nombre.substr(0, 1).toUpperCase() );
								}
							}).then(imgBlob => {
			    					...
			    					<table id="dataTable" className="table nowrap server-side table-hover" ref="grid" width="100%">
											<tfoot>
												<tr style={{backgroundColor: '#fff'}}>
													...
													<th className="dt-search-header">Marca</th>
