...
class ModeloTiposGrid extends Component {
	constructor(props) {
		super(props);
        this.dataService = '/modelo-tipos/grid';
				...
				columns: [
					...
					drawCallback: function () {
						$(this).find('.action').on('click', function () {
							...
							.then((isConfirm) => {
								if (isConfirm) {
									fetch(Config.get('apiUrlBase') + '/modelo-tipos/' + data.id, {
										method: 'DELETE',
										headers: {
...
export default ModeloTiposGrid;
