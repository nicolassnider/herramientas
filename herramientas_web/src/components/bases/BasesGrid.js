				...
				columns: [
					...
					drawCallback: function () {
						$(this).find('.action').on('click', function () {
							...
							.then((isConfirm) => {
								if (isConfirm) {
									fetch(Config.get('apiUrlBase') + '/bases/' + data.id, {
										method: 'DELETE',
										headers: {
			    					...
			    					<table id="dataTable" className="table nowrap server-side table-hover" ref="grid" width="100%">
											<tfoot>
												<tr style={{backgroundColor: '#fff'}}>
													...
													<th className=""></th>
													<th className="p-0"></th>
													<th className="dt-search-header">Base</th>
													<th className="dt-search-header">Región</th>
													<th className="dt-search-header">Sub Región</th>
													<th className="dt-search-header">Jefe</th>
													<th className="dt-search-header">Responsable</th>
