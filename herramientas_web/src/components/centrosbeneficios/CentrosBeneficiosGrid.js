...
class CentrosBeneficiosGrid extends Component {
	constructor(props) {
		super(props);
        this.dataService = '/centros-beneficios/grid';
				...
				columns: [
					{
						...
						render: function (data, type, row) {
							return '<div class="dt-user-avatar">'+row.nombre.substr(0,1).toUpperCase()+'</div>';
						},
						filterType: 'none'
					},{
					...
					drawCallback: function () {
						$(this).find('.action').on('click', function () {
							...
							.then((isConfirm) => {
								if (isConfirm) {
									fetch(Config.get('apiUrlBase') + '/centros-beneficios/' + data.id, {
										method: 'DELETE',
										headers: {
			    					...
			    					<table id="dataTable" className="table nowrap server-side table-hover" ref="grid" width="100%">
											<tfoot>
												<tr style={{backgroundColor: '#fff'}}>
													...
													<th className=""></th>
													{/*<th className="p-0"></th>*/}
													<th className="dt-search-header">Centro de Costo</th>
													<th className="dt-search-header">Descripción</th>
...
export default CentrosBeneficiosGrid;
